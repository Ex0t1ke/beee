...
import { playChimeSuccess } from '../utils/sound';
...
export const UpgradesPanel: React.FC<UpgradesPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'click' | 'passive' | 'wardrobe' | 'achievements'>('click');
  ...
  // Count available affordable upgrades for badge
  const affordableCount = upgrades.filter((u) => wool >= u.cost).length;
        ...
        {isOpen && (
          <>
            ...
            >
              ...
              <div className="p-4 bg-amber-800 text-amber-50 flex items-center justify-between border-b border-amber-900">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-200">Улучшения для овечки и загона</p>
                  </div>
                </div>
              ...
              {/* Tabs Navigation */}
              <div className="flex border-b border-amber-200 bg-amber-100/60 p-1.5 gap-1 text-xs font-bold">
                <button
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    ...
                    {clickUpgrades.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <span className="block">{formatNumber(item.cost)}</span>
                            <span className="text-[9px] font-normal block opacity-85">шерсти</span>
                          </button>
                        </div>
                      );
                    })}
                ...
                {/* 2. Passive Corral Upgrades */}
                {activeTab === 'passive' && (
                  <>
                    <div className="text-xs text-stone-500 mb-2 px-1">
                      Улучшения загона автоматически дают шерсть каждую секунду.
                    </div>
                ...
                {activeTab === 'wardrobe' && (
                  <>
                    <div className="text-xs text-stone-500 mb-2 px-1">
                      Украшения открываются при достижении рубежей собранной шерсти!
                    </div>
                    {accessories.map((item) => {
                      const isUnlocked = wool >= item.unlockedAtWool || item.unlockedAtWool === 0;
                      const isCurrent = activeAccessory === item.id;
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-11 h-11 rounded-xl bg-amber-100/70 flex items-center justify-center text-2xl border border-amber-200">
                              {item.icon}
                            </div>
                            <div>
                              <h4 className="font-bold text-sm text-stone-800">{item.name}</h4>
                              ...
                              {!isUnlocked && (
                                <p className="text-[11px] font-semibold text-amber-700 mt-0.5">
                                  Требуется: {formatNumber(item.unlockedAtWool)} шерсти
                                </p>
                              )}
                            </div>
                          </div>
                          ...
                          {isCurrent ? (
                            <span className="flex items-center gap-1 text-xs font-bold text-emerald-700 bg-emerald-100 px-3 py-1.5 rounded-xl">
                ...
                {activeTab === 'achievements' && (
                  <>
                    <div className="text-xs text-stone-500 mb-2 px-1">
                      Достижения фермы за сбор шерсти и заботу об овечке.
                    </div>
